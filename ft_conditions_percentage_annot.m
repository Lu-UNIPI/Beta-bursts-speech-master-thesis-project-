...
for i=5%1:numel(SUBJECTS)  
    ...
    prod_triplet=prod_triplet(prod_triplet.session_id==id_session,{'id', 'starts','ends','duration','stim_volume','phonetic_ontarget'});
    ...
    if any(strcmp(choice,{'all','dbs'})) && strcmp(SUBJECT,"DBS3028")
        cfg=[];
    ...
    baselinetype='percentage'; % 'difference', 'percentage', 'db', 'zscore'
    for e=1:nElec
        disp('---------------------------------------------------------')
        ...
        for j=1:numel(criteria)
            ...
            for k=1:numel(cond_definition)
                ...
                powtab.speech_beta_rebound=mean(pow_beta(:, idx1:idx2),'all','omitnan');
                powtab.speech_highbeta_rebound=mean(pow_highbeta(:, idx1:idx2),'all','omitnan');
                powtab.speech_lowbeta_rebound=mean(pow_lowbeta(:, idx1:idx2),'all','omitnan');
                    % stim
                ...
                [~,idx2]=min(abs(Etoplot.time-(x_stim3_end+1)));
                powtab.stim_beta_rebound=mean(pow_beta(:, idx1:idx2),'all','omitnan');
                powtab.stim_highbeta_rebound=mean(pow_highbeta(:, idx1:idx2),'all','omitnan');
                powtab.stim_lowbeta_rebound=mean(pow_lowbeta(:, idx1:idx2),'all','omitnan');
    ...
    powtab_accp=powtab_accp(~powtab_accp.id==0, :);powtab_accn=powtab_accn(~powtab_accn.id==0, :);powtab_accd=powtab_accd(~powtab_accd.id==0, :);
    ...
    powtab_accp.id=(1:height(powtab_accp))';powtab_accn.id=(1:height(powtab_accn))';powtab_accd.id=(1:height(powtab_accd))';
    if cc==1
        writetable(powtab_volp, strcat('annot/conditions CTAR/',SUBJECT,'_speech_normperc_volume_pos.txt'), 'Delimiter', '\t');
        writetable(powtab_voln, strcat('annot/conditions CTAR/',SUBJECT,'_speech_normperc_volume_neg.txt'), 'Delimiter', '\t');
        writetable(powtab_vold, strcat('annot/conditions CTAR/',SUBJECT,'_speech_normperc_volume_diff.txt'), 'Delimiter', '\t');
        writetable(powtab_rtp, strcat('annot/conditions CTAR/',SUBJECT,'_speech_normperc_reacttime_pos.txt'), 'Delimiter', '\t');
        writetable(powtab_rtn, strcat('annot/conditions CTAR/',SUBJECT,'_speech_normperc_reacttime_neg.txt'), 'Delimiter', '\t');
        writetable(powtab_rtd, strcat('annot/conditions CTAR/',SUBJECT,'_speech_normperc_reacttime_diff.txt'), 'Delimiter', '\t');
        writetable(powtab_accp, strcat('annot/conditions CTAR/',SUBJECT,'_speech_normperc_accuracy_pos.txt'), 'Delimiter', '\t');
        writetable(powtab_accn, strcat('annot/conditions CTAR/',SUBJECT,'_speech_normperc_accuracy_neg.txt'), 'Delimiter', '\t');
        writetable(powtab_accd, strcat('annot/conditions CTAR/',SUBJECT,'_speech_normperc_accuracy_diff.txt'), 'Delimiter', '\t');
    elseif cc==2
        writetable(powtab_volp, strcat('annot/conditions/',SUBJECT,'_speech_normperc_volume_pos.txt'), 'Delimiter', '\t');
        writetable(powtab_voln, strcat('annot/conditions/',SUBJECT,'_speech_normperc_volume_neg.txt'), 'Delimiter', '\t');
        writetable(powtab_vold, strcat('annot/conditions/',SUBJECT,'_speech_normperc_volume_diff.txt'), 'Delimiter', '\t');
        writetable(powtab_rtp, strcat('annot/conditions/',SUBJECT,'_speech_normperc_reacttime_pos.txt'), 'Delimiter', '\t');
        writetable(powtab_rtn, strcat('annot/conditions/',SUBJECT,'_speech_normperc_reacttime_neg.txt'), 'Delimiter', '\t');
        writetable(powtab_rtd, strcat('annot/conditions/',SUBJECT,'_speech_normperc_reacttime_diff.txt'), 'Delimiter', '\t');
        writetable(powtab_accp, strcat('annot/conditions/',SUBJECT,'_speech_normperc_accuracy_pos.txt'), 'Delimiter', '\t');
        writetable(powtab_accn, strcat('annot/conditions/',SUBJECT,'_speech_normperc_accuracy_neg.txt'), 'Delimiter', '\t');
        writetable(powtab_accd, strcat('annot/conditions/',SUBJECT,'_speech_normperc_accuracy_diff.txt'), 'Delimiter', '\t');
    end
end
end
