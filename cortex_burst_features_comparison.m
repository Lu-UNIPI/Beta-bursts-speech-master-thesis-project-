...
%% analisi
measures={'n_bursts','perc_bursts'};%'duration','volt_amp','time_rdsym','time_ptsym','frequency',
...
for m=1:numel(measures)
    ...
    for i=ii
        ...
        % get BURSTS DATA
        tab=readtable(strcat('annot/general CTAR/areas/',SUBJECT," bycycle features comparison.txt"));
        alldata{i}=tab(strcmp(tab.measure,measure),:);
    end
...
%     min_value = limits.min_dbs;max_value = limits.max_dbs;
    ...
    saveas(fig1,strcat('images/bursts CTAR/features comparison/',measures{m}," ",toEvaluate,'.png'));
    saveas(fig1,strcat('images/bursts CTAR/features comparison/',measures{m}," ",toEvaluate,'.fig'));
    close all
