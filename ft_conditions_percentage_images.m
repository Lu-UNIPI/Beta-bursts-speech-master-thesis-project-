...
for i=1:numel(SUBJECTS)  
    ...
    prod_triplet=prod_triplet(prod_triplet.session_id==id_session,{'id', 'starts','ends','duration','stim_volume'});
...
%% select ecog and referencing
    ...
    for e=1:nElec
        ...
        if mean(Esingle_mor.powspctrm,'all','omitnan')==0;disp('Electrode rejected, going to the next');continue;end
        ...
        sgtitle(tit)
        [cmin,cmax]=caxis;
        if cc==1;name=[strcat('images/conditions CTAR/',SUBJECT,'_speech_',strrep(string(Esingle_norm.label),'_',''),'_normperc')];
        elseif cc==2;name=[strcat('images/conditions/',SUBJECT,'_speech_',strrep(string(Esingle_norm.label),'_',''),'_normperc')];
        end
